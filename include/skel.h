...
// struct for an arp entry in the arp_table
struct arp_entry {
	__u32 ip;
	uint8_t mac[6];
};

// struct for a router entry in the router_table
 ...
 */
void send_arp(uint32_t daddr, uint32_t saddr, struct ether_header *eth_hdr, int interface, uint16_t arp_op1);
/**
 * @brief 
 * 
 * @param buffer 
...
/* function to automatically build the ethernet header */
